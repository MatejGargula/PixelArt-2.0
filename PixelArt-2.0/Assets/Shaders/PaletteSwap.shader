Shader"PixelArt_2.0/PaletteSwap"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                const half4 main = i.color * SAMPLE_TEXTURE2D_LOD(_MainTex, sampler_MainTex, i.uv, 0);
                ...
                float3 lab = RGB_to_Lab(colorOut.rgb);

                // Nearest color search over 1D palette texture
                ...
                for (int k = 0; k < count; ++k)
                {
                    ...
                    float d = DeltaE2(lab, pLab);
                    if (d < bestD) { bestD = d; bestI = k; }
                }

                float uBest = (bestI + 0.5) / count;
                float3 outRGB = SAMPLE_TEXTURE2D_LOD(_PaletteTex, sampler_PaletteTex, float2(uBest, 0.5), 0).rgb;

                // Feed remapped color into URP 2D lighting
                float4 remap = float4(outRGB, colorOut.a);    
