Shader"PixelArt_2.0/MonoPaletteSwap"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                const half3 LUMA_WEIGHTS = half3(0.2126h, 0.7152h, 0.0722h);
                half lum = dot(inRGB, LUMA_WEIGHTS); // can be slightly outside [0,1] so...

                half t = saturate(lum);

                int count = max(_PaletteCount, 1);
    
                half scaled = t * (half) count; 
                int idx = (int) floor(scaled); 
                idx = min(idx, count - 1); 

                half u = ((half) idx + 0.5h) / (half) count;
                half3 paletteRGB = SAMPLE_TEXTURE2D_LOD(
                    _PaletteTex, sampler_PaletteTex,
                    float2(u, 0.5), 0
                ).rgb;
